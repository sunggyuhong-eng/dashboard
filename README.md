...
- 별도 `지원자 추이` 화면에서 게임잡의 날짜별·프로젝트별·공고별 지원 유입 확인
...
게임잡 지원자 추이 자동화를 사용하려면 같은 화면에 다음 Repository Secret도 등록합니다.

- `GAMEJOB_ID`: 게임잡 기업회원 아이디
- `GAMEJOB_PASSWORD`: 게임잡 기업회원 비밀번호

계정값은 코드나 JSON에 저장하지 않으며 GitHub Actions 실행 중 로그인에만 사용됩니다. 워크플로 로그에는 아이디·비밀번호와 지원자 개인정보를 출력하지 않습니다.
...
이름·사진·전화번호·이메일·이력서 내용은 JSON에 저장하지 않습니다. 공고 목록과 지원자 목록 모두 다음 페이지가 없을 때까지 자동 순회합니다.
